...
my $output_file = $output_dir . "mhci_ml.txt";
my $threshold = 50;
my $ignore_CombLib = 1;
my $get_headers = 1;
...
while (defined (my $line = <INPUT_FILE>)) {
	...
	# Get binding info				
		...
		if ($get_headers == 1) {
			my ($mhc, $length, $species) = split ('\s+',$1);
					
			$header = $header . "," . $mhc . "-" . $length;
	...
	# Ignore line if no matching species				
		...
		if ($Percentile_Rank < $min_Percentile) {
			...
			$min_percentile_scores = $ANN_IC50 . "," . $SMM_IC50 . "," . $CombLib_IC50 . "," . $NetMHCpan_IC50;
		}	
...
sub add_average_score ()
{
	...
	my ($ANN_score,$SMM_score,$CombLib_score,$NetMHCpan_score) = split (',', $min_percentile_scores);
	...
	my @scores = ($ANN_score,$SMM_score,$CombLib_score,$NetMHCpan_score);
